...
class ChurchScheduleView extends StatefulWidget {
  const ChurchScheduleView({super.key});
...
class _ChurchScheduleViewState extends State<ChurchScheduleView>
    with TableFocusNodeMixin<ChurchScheduleView, ContentMd> {
  @override
    ...
    return DefaultTable(
        headerEnd: SpacedRow(
          ...
          children: [
            ...
            ElevatedButton(
                onPressed: () {
                  onEdit((p0) => ChurchScheduleCreateNewPopup(model: p0), null);
                },
        ...
        PlutoColumn(
          ...
          renderer: (rendererContext) {
            return rendererContext.actionMenuWidget(
              onEdit: () {
                onEdit((p0) => ChurchScheduleCreateNewPopup(model: p0),
  ...
  Future<List<ContentMd>?> fetch() async {
    final success = await dependencyManager.firestore.getChurchSchedule();
    if (success.isLeft) {
      return success.left;
    } else {
  ...
  Future<bool> deleteSelected(PlutoRow? singleRow) async {
    ...
    for (final row in selected) {
      ...
      final res = await dependencyManager.firestore.deleteChurchSchedule(id);
      if (res.isLeft) {
