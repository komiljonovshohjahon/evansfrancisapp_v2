...
class _UaeYtViewState extends State<UaeYtView>
    with TableFocusNodeMixin<UaeYtView, YoutubeContentMd> {
  @override
    ...
    return DefaultTable(
        headerEnd: SpacedRow(
          ...
          children: [
            ...
            ElevatedButton(
                onPressed: () {
                  onEdit((p0) => UaeYtCreateNewPopup(model: p0), null);
                },
        ...
        PlutoColumn(
          ...
          renderer: (rendererContext) {
            return rendererContext.actionMenuWidget(
              onEdit: () {
                onEdit((p0) => UaeYtCreateNewPopup(model: p0),
  ...
  Future<List<YoutubeContentMd>?> fetch() async {
    final success = await dependencyManager.firestore.getUaeYoutube();
    if (success.isLeft) {
      return success.left;
    } else {
