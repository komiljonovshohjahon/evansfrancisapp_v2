...
class _ContactChurchOfficeViewState extends State<ContactChurchOfficeView>
    with TableFocusNodeMixin<ContactChurchOfficeView, ChurchContactMd> {
  @override
  List<PlutoColumn> get columns => [
        ...
        PlutoColumn(
          ...
          renderer: (rendererContext) {
            final model =
                rendererContext.row.cells['action']!.value as ChurchContactMd;
            return TextButton(
  ...
  void onReview(ChurchContactMd model) {
    context.futureLoading(() async {
      final success = await dependencyManager.firestore
          .createOrUpdateContactChurch(
  ...
  void deleteSelected(PlutoRow? singleRow) {
    onDelete(() async {
      ...
      for (final row in selected) {
        ...
        final res = await dependencyManager.firestore.deleteContactChurch(id);
        if (res.isLeft) {
  ...
  @override
  PlutoRow buildRow(ChurchContactMd model) {
  ...
  @override
  Future<List<ChurchContactMd>?> fetch() async {
    final success = await dependencyManager.firestore.getChurchContacts();
    if (success.isLeft) {
      return success.left;
